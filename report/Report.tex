...
\begin{algorithm}
	...
	\While{$i < \numIt$}
		...
		\State $r \gets \sign \braces{\grad[0]}$
		\State \advEx $\ \gets$ $\advEx + \frac{c}{\numIt} \cdot r$
		\State $i \gets i + 1$
	\EndWhile
	\State \Return \advEx
	\EndFunction
...
With the method presented in section \ref{sec:generation-of-ae}, it is possible to create convincing adversarial examples that both trick the network into predicting a false class, and are indistinguishable from regular images by humans at the same time. How well it works depends on the original image, target class and careful choice of the parameters and will be below later, but for now a functional example is given in figure \ref{fig:ae-example}.
...
\begin{figure}[htp]
    ...
    \end{subfigure}
    \caption{Illustration of the deviation between the panda in figure \ref{fig:ae-example-original} and an adversarial example mimiking the class represented by \ref{fig:imagenet-acorn-dresser-dresser}. AE was generated in $10$ iterations with $c = 0.9$.}
...
\begin{figure}[htp]
    ...
    \end{subfigure}
    \caption{Illustration of the deviation between the panda in figure \ref{fig:ae-example-original} and an adversarial example mimiking the class represented by \ref{fig:imagenet-acorn-dresser-acorn}. AE was generated in $10$ iterations with $c = 3.0$.}
...
\subsection{The effects of iterations and step size}
% TODO 94.49 fix discrepancy with table
...
\begin{figure}[htp]
	...
	\caption{Plot of the network confidence at the end of AE generation as a function of iteration count}
	\label{fig:confidence-vs-iterations}
\end{figure}

% TODO finish entering data
\begin{table}[htp]
	\begin{tabular}{|l|llllllll|}
		\hline
		Iteration & 10 & 25 & 50 & 75 & 100 & 125 & 150 & 175 \\
		\hline
		Acorn confidence & 0.8613 & 0.9258 & 0.9392 & 0.9427 & 0.9455 & 0.9462 & 0.946935 & 0.9475 \\
		Dresser confidence & 0.01144 & 0.01576 & 0.01774 & 0.01832 & 0.01867 & 0.01899 & 38 & 39 \\
		\hline
	\end{tabular}
	\caption{Table of the network confidence at the end of AE generation as a function of iteration count}
	\label{tab:confidence-vs-iterations}
\end{table}

% TODO 500... did i enter all the data so plots and tables show 500?
...
% plot of confidence vs. iterations
% plot of confidence vs. coeff
