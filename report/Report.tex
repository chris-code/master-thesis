...
\subsection{Analysis of fundamental properties}
With this method, it is possible to create convincing adversarial examples that both trick the network into predicting a false class, and are indistinguishable from regular images by humans at the same time. How well it works depends on the original image, target class and careful choice of the parameters and is analyzed later, but for now a functional example is given in figure \ref{fig:ae-example}.
...
\begin{figure}[htp]
    ...
    \end{subfigure}
    \caption{Illustration of the deviation between the images in figure \ref{fig:ae-example}. Images were scaled by the inverse of their 98th percentile to increase visibility.}
...
% Compare difference with gradient
% 100 iterations
